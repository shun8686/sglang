...
if [[ "$LOCAL_HOST1" == "${P_IP[0]}" || "$LOCAL_HOST2" == "${P_IP[0]}" ]]; then
    echo "Starting Prefill on ${P_IP[0]}"
    ...
    # 单机Prefill：TP_SIZE根据实际单机卡数调整（示例为16卡）
    python -m sglang.launch_server --model-path ${MODEL_PATH} --disaggregation-mode prefill \
    --host ${P_IP[0]} --port 32125 --disaggregation-bootstrap-port 8998 \
    ...
    --max-running-requests 40 --chunked-prefill-size -1 --max-prefill-tokens 128000 \
...
# ================== Decode 节点（单机单IP） ==================
if [[ "$LOCAL_HOST1" == "${D_IP[0]}" || "$LOCAL_HOST2" == "${D_IP[0]}" ]]; then
    echo "Starting Decode on ${D_IP[0]}"
    export ASCEND_MF_STORE_URL="tcp://${P_IP[0]}:24669"
    ...
    # 单机Decode：去掉dist-init-addr，nnodes=1，TP/EP_SIZE根据实际单机卡数调整（示例为16卡）
    python -m sglang.launch_server --model-path ${MODEL_PATH} --disaggregation-mode decode \
    --host ${D_IP[0]} --port 33125 --trust-remote-code \
    --nnodes 1 --node-rank 0 \
    --tp-size 16 --ep-size 16 \
    --mem-fraction-static 0.75 --max-running-requests 192 \
    --attention-backend ascend --device npu \
    --enable-multimodal --quantization modelslim \
    --moe-a2a-backend deepep --enable-dp-attention --deepep-mode low_latency \
    --enable-dp-lm-head --dp-size 4 --enable-dp-attention \
    --cuda-graph-bs 8 16 24 32 40 48 --disaggregation-transfer-backend ascend --watchdog-timeout 9000 \
