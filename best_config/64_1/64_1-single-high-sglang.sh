...
--device npu \
--tp-size 16 \
--chunked-prefill-size -1 --max-prefill-tokens 65536 --prefill-max-requests 1 \
--disable-radix-cache \
--trust-remote-code \
--host 127.0.0.1 --max-running-requests 32 \
--mem-fraction-static 0.57 --max-total-tokens 1065000 \
--port 31125 \
