...
do
	...
	--trust-remote-code --nnodes 1 --node-rank 0 \
        --tp-size 16 --mem-fraction-static 0.6 \
	--attention-backend ascend --device npu \
	--enable-multimodal --quantization modelslim \
        --disaggregation-transfer-backend ascend \
	--max-running-requests 64 --chunked-prefill-size -1 --max-prefill-tokens 65536 \
	...
	--mm-enable-dp-encoder --disable-radix-cache
        NODE_RANK=$i
        break
    fi
done
...
do
	...
	export ASCEND_MF_STORE_URL="tcp://${P_IP[0]}:24669"
        ...
        export HCCL_BUFFSIZE=2000
        export SGLANG_DEEPEP_NUM_MAX_DISPATCH_TOKENS_PER_RANK=136
	...
	--tp-size 32 --ep-size 32 \
        --mem-fraction-static 0.6 --max-running-requests 192 \
	--attention-backend ascend --device npu \
	--enable-multimodal --quantization modelslim \
        --moe-a2a-backend deepep --enable-dp-attention --deepep-mode low_latency \
	--enable-dp-lm-head --dp-size 4 --enable-dp-attention \
        --cuda-graph-bs 2 4 8 12 16 18 20 24 28 32 34 40 48 --disaggregation-transfer-backend ascend --watchdog-timeout 9000 \
	...
	--speculative-num-steps 3 --speculative-eagle-topk 1 --speculative-num-draft-tokens 4 \
        ...
        --load-balance-method round_robin --disable-radix-cache --max-total-tokens 200000
        NODE_RANK=$i
        break
    fi
done
