...
from sglang.test.ascend.test_ascend_utils import (
    QWEN3_8B_EAGLE3_YZY_WEIGHTS_PATH,
    QWEN3_8B_YZY_WEIGHTS_PATH,
    run_command,
)
...
class TestDraftConfigFile(CustomTestCase):
    ...
    def setUpClass(cls):
        ...
        run_command(
            f"mv {os.path.join(QWEN3_8B_YZY_WEIGHTS_PATH, 'config.json')} {os.path.join(QWEN3_8B_YZY_WEIGHTS_PATH, '_config.json')}"
        )
        run_command(
            f"mv {os.path.join(QWEN3_8B_EAGLE3_YZY_WEIGHTS_PATH, 'config.json')} {os.path.join(QWEN3_8B_EAGLE3_YZY_WEIGHTS_PATH, '_config.json')}"
        )
        try:
            ...
            run_command(
                f"mv {os.path.join(QWEN3_8B_YZY_WEIGHTS_PATH, '_config.json')} {os.path.join(QWEN3_8B_YZY_WEIGHTS_PATH, 'config.json')}"
            )
            run_command(
                f"mv {os.path.join(QWEN3_8B_EAGLE3_YZY_WEIGHTS_PATH, '_config.json')} {os.path.join(QWEN3_8B_EAGLE3_YZY_WEIGHTS_PATH, 'config.json')}"
            )
            if cls.process:
