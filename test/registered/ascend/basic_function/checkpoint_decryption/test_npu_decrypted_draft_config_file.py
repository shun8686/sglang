...
from sglang.test.ascend.test_ascend_utils import (
    YZY_QWEN3_8B_EAGLE3_WEIGHTS_PATH,
    YZY_QWEN3_8B_WEIGHTS_PATH,
    run_command,
)
...
class TestDraftConfigFile(CustomTestCase):
    ...
    def setUpClass(cls):
        ...
        run_command(
            f"mv {os.path.join(YZY_QWEN3_8B_WEIGHTS_PATH, 'config.json')} {os.path.join(YZY_QWEN3_8B_WEIGHTS_PATH, '_config.json')}"
        )
        run_command(
            f"mv {os.path.join(YZY_QWEN3_8B_EAGLE3_WEIGHTS_PATH, 'config.json')} {os.path.join(YZY_QWEN3_8B_EAGLE3_WEIGHTS_PATH, '_config.json')}"
        )
        try:
            ...
            run_command(
                f"mv {os.path.join(YZY_QWEN3_8B_WEIGHTS_PATH, '_config.json')} {os.path.join(YZY_QWEN3_8B_WEIGHTS_PATH, 'config.json')}"
            )
            run_command(
                f"mv {os.path.join(YZY_QWEN3_8B_EAGLE3_WEIGHTS_PATH, '_config.json')} {os.path.join(YZY_QWEN3_8B_EAGLE3_WEIGHTS_PATH, 'config.json')}"
            )
            if cls.process:
    ...
    def test_gsm8k(self):
        args = SimpleNamespace(
            ...
            model=YZY_QWEN3_8B_WEIGHTS_PATH,
            eval_name="gsm8k",
            api="completion",
            num_examples=200,
            num_threads=128,
