...
class TestTwoBatchOverlap(unittest.TestCase):
    """Test class for two-batch overlap optimization on DeepSeek-Coder model.

    Core Purpose:
    - Verify two-batch overlap functionality with TP=2/DM=2 parallelism on DeepEP backend
    - Validate basic inference correctness and MMLU accuracy threshold
    - Ensure compatibility with MoE-A2A backend (deepep) and two-batch overlap optimization
    """
...
class TestTwoBatchOverlapUnitTest(unittest.TestCase):
    """Unit test class for two-batch overlap core calculation functions.

    Test Targets:
    - compute_split_seq_index: Calculate sequence split index for batch overlap
    - compute_split_token_index: Calculate token split index for batch overlap
    - Cover DECODE/EXTEND forward modes with various input scenarios
    """
