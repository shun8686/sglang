#!/bin/bash
# ============================================================
# NPU Pre-check Script (Inside Container)
# ============================================================
# This script performs environment checks before executing NPU
# test cases. It must be run INSIDE the NPU Docker container.
#
# Usage:
#   ./pre_check.sh [model_path]
#
# Checks performed:
#   1. NPU card availability (npu-smi info)
#   2. Identify completely idle cards (no running processes)
#   3. Map idle NPU IDs to chip IDs (for ASCEND_RT_VISIBLE_DEVICES)
#   4. Verify model path exists (optional)
#
# Exit codes:
#   0 - All checks passed, usable chips printed to stdout
#   1 - Fatal error (npu-smi failed)
...
# -----------------------------------------------------------
# Step 1: Check NPU card status
# -----------------------------------------------------------
info "Step 1/4: Running npu-smi info..."

NPU_OUTPUT=$(npu-smi info 2>&1) || {
    fail "Failed to run npu-smi info"
    echo "$NPU_OUTPUT"
    exit 1
}
...
# -----------------------------------------------------------
# Step 2: Identify completely idle cards
# -----------------------------------------------------------
info "Step 2/4: Identifying completely idle NPU cards..."
...
# -----------------------------------------------------------
# Step 3 (optional): Verify model path
# -----------------------------------------------------------
if [ -n "$MODEL_PATH" ]; then
    info "Step 3/4: Verifying model path '$MODEL_PATH'..."
    if [ -d "$MODEL_PATH" ] && [ -f "$MODEL_PATH/config.json" ]; then
        pass "Model path '$MODEL_PATH' exists and contains config.json"
    else
        # Try to list available models
        available_models=$( { ls "$MODEL_PATH" 2>/dev/null; ls "$(dirname "$MODEL_PATH")" 2>/dev/null; } | head -20)
    ...
    info "Step 3/4: Skipping model path check (no model path provided)"
fi

# -----------------------------------------------------------
# Step 4: Print recommended chip assignment
# -----------------------------------------------------------
info "Step 4/4: Generating chip assignment recommendations..."
