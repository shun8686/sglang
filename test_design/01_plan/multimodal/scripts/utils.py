...
Import from this module instead of duplicating helper functions across
test files.  Each helper has zero or minimal external dependencies.
"""
...
# ============================================================
# Color enum for test image creation
# ============================================================
...
# ============================================================
# Model paths (local definitions not in test_ascend_utils)
# ============================================================

MODEL_WEIGHTS_DIR = "/root/.cache/modelscope/hub/models"

QWEN3_5_9B_PATH = os.path.join(MODEL_WEIGHTS_DIR, "Qwen/Qwen3.5-9B")
QWEN3_5_35B_A3B_PATH = os.path.join(MODEL_WEIGHTS_DIR, "Qwen/Qwen3.5-35B-A3B")
QWEN3_5_35B_A3B_W8A8_MTP_PATH = os.path.join(
    MODEL_WEIGHTS_DIR, "Qwen/Qwen3.5-35B-A3B-w8a8-mtp"
)
QWEN3_VL_LORA_PATH = os.path.join(MODEL_WEIGHTS_DIR, "Qwen/Qwen3-VL-4B-Lora")
QWEN3_VL_4B_EAGLE3_PATH = os.path.join(
    MODEL_WEIGHTS_DIR, "Qwen/Qwen3-VL-4B-Instruct_eagle3"
)
QWEN3_VL_8B_EAGLE3_PATH = os.path.join(
    MODEL_WEIGHTS_DIR, "Qwen/Qwen3-VL-8B-Instruct-Eagle3"
)
QWEN3_6_27B_PATH = os.path.join(MODEL_WEIGHTS_DIR, "Qwen/Qwen3.6-27B")
...
def create_test_image(
    ...
    """
    rgb = _COLOR_MAP[_COLOR_NAME_MAP.get(color, color)]
...
def get_port(offset):
    """Return a port number unique to this CI job + test offset.

    Uses ``ASCEND_RT_VISIBLE_DEVICES`` to avoid collisions between
    parallel CI jobs that share the same host.
    """
    devices = os.environ.get("ASCEND_RT_VISIBLE_DEVICES", "0")
    device_idx = int(devices.split(",")[0])
    return 30000 + device_idx * 100 + offset
...
def assert_color_and_shape(test_case, text, expected_color, expected_shape, prefix=""):
    ...
    Args:
        ...
        expected_shape: Expected shape string (e.g. ``"ellipse"``,
            ``"rectangle"``).
        prefix: Optional label prepended to failure messages (e.g.
            ``"P2-004: "``).
...
# ============================================================
# Common NPU server args & launcher
# ============================================================
...
def launch_server(model, port, extra_args=None, timeout=None, env=None, extra_env=None):
