...
**Multi-server** (baseline + speculative, each server TP=1, no extra workers): `--base-gpu-id` indexes into `ASCEND_RT_VISIBLE_DEVICES` — **1 NPU (2 chips) suffices for 2 servers**.

```
ASCEND_RT_VISIBLE_DEVICES=10,11
→ Server 1 (baseline): --base-gpu-id 1  (chip 11)
→ Server 2 (spec):     --base-gpu-id 0  (chip 10)
```

Only escalate to 2 NPUs (4 chips) when TP≥2, extra workers (e.g. EAGLE draft), or the test explicitly sets different absolute GPU indices.

## Stop Server

```bash
docker exec <container> bash -c "pkill -9 -f sglang 2>/dev/null; sleep 2; ps aux | grep sglang | grep -v grep | wc -l"
```
