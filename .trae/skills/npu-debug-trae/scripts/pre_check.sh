...
echo "  Two-server (baseline + speculative, TP=1 each):"
echo "    ASCEND_RT_VISIBLE_DEVICES=\"${first_chips}\""
echo "    -> Server 1 (spec):     --base-gpu-id 0 (chip ${first_chips%%,*})"
echo "    -> Server 2 (baseline): --base-gpu-id 1 (chip ${first_chips##*,})"
echo "    NOTE: --base-gpu-id indexes into the chip LIST, not absolute ID."
echo "          Only escalate to 2 NPUs when TP>=2 or extra workers needed."
echo ""
